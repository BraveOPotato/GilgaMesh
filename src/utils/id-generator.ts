...
// ─── HUMAN-READABLE IDS ───────────────────────────────────────────────────────
// Ported verbatim from ids.js — same word lists, same algorithm.
...
export function generatePeerId(): PeerId {
  ...
  return `${adj}-${noun}-${String(Math.floor(Math.random() * 9000) + 1000)}` as PeerId;
}

export function generateRoomId(): RoomId {
  return String(Math.floor(Math.random() * 9000) + 1000) as RoomId;
}

/** Short random ID for messages, file tokens, etc. */
