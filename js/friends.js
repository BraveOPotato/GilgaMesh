...
// ─── VERIFY TOKEN ─────────────────────────────────────────────────────────────
// Each friendship has a SHARED SECRET generated by the initiator and stored
// identically on both sides.  Neither side can impersonate the other to a
// third party because each friendship has a different secret.
//
// On every connection open, each peer sends:
//   { type: 'verify_token', token: <sharedSecret for THIS peer>, id, name }
//
// Receiver logic:
//   1. Scan all friend records for a matching sharedSecret.
//      a. Match found, same peer ID   → verified (clear impostor flag if set)
//      b. Match found, DIFFERENT pid  → peer rotated ID; update record silently
//      c. No match, but pid IS a friend → wrong secret → impostor warning
//      d. No match, pid unknown        → stash for when/if they become a friend
export function handleVerifyToken(data, conn) {
  const remotePid   = data.id || conn.peer;
  const remoteToken = data.token;
  const remoteName  = data.name || remotePid;
  if (!remoteToken || !remotePid) return;
  if (!state.friends) state.friends = {};

  // Search all friends for this shared secret (O(n) over friends list — fine)
  const matchEntry = Object.entries(state.friends).find(
  ...
  if (matchEntry) {
    ...
    if (storedPid === remotePid) {
      // Case 1a — correct peer, correct secret → verified
      if (friend.impostor) {
        friend.impostor = false;
        saveFriendsData();
        if (state.activeFriendsView) { renderFriendsSidebar(); renderFriendsGrid(); }
      }
    } else {
      // Case 1b — secret matches but peer ID changed → ID rotation
      const updated = { ...friend, id: remotePid, name: remoteName, impostor: false };
      delete state.friends[storedPid];
      state.friends[remotePid] = updated;
      saveFriendsData();
      if (state.activeFriendsView) { renderFriendsSidebar(); renderFriendsGrid(); }
      toast(`${remoteName} reconnected with a new peer ID (identity verified ✓)`, 'info');
    }
    return;
  }

  // No secret match — is this a known friend?
  const knownFriend = state.friends[remotePid];
  if (knownFriend) {
    if (!knownFriend.sharedSecret) {
      // Friend exists but no secret yet (legacy / pre-feature record) — trust first token
      knownFriend.sharedSecret = remoteToken;
      knownFriend.impostor = false;
      saveFriendsData();
    } else {
      // Case 1c — known friend, wrong secret → possible impostor
      knownFriend.impostor = true;
      saveFriendsData();
      if (state.activeFriendsView) { renderFriendsSidebar(); renderFriendsGrid(); }
      toast(`⚠️ ${remoteName} connected with an unexpected identity token`, 'error');
    }
    return;
  }

  // Case 1d — unknown peer; stash token so we can match on future friending
  if (!state._pendingTokens) state._pendingTokens = {};
  state._pendingTokens[remotePid] = remoteToken;
}

// Called internally when a friendship is confirmed.
// If the other peer already sent us a verify_token before we friended them,
// their token is sitting in _pendingTokens — pull it in as sharedSecret.
// (Only used as fallback; normally the initiator sets sharedSecret directly.)
export function attachPendingToken(peerId) {
  if (!state._pendingTokens?.[peerId]) return;
  if (!state.friends?.[peerId]) return;
  if (!state.friends[peerId].sharedSecret) {
    state.friends[peerId].sharedSecret = state._pendingTokens[peerId];
  }
  delete state._pendingTokens[peerId];
  saveFriendsData();
}
...
export function addFriend(peerId, name) {
  ...
  // Generate the shared secret for this friendship — stored on both sides
  const sharedSecret = Math.random().toString(36).slice(2) +
                       Math.random().toString(36).slice(2) +
                       Date.now().toString(36);
  // Stash it so _addFriendConfirmed (called on accept) can pick it up
  if (!state._pendingTokens) state._pendingTokens = {};
  state._pendingTokens[peerId] = sharedSecret;

  const reqId = genId();
  const wire = (conn) => {
    try {
      conn.send({ type: 'friend_request', reqId, from: state.myId, fromName: state.myName, sharedSecret });
...
export function _addFriendConfirmed(peerId, name) {
  ...
  state.friends[peerId] = {
    id: peerId, name: name || peerId, addedAt: Date.now(),
    nickname: existing?.nickname || '',
    sharedSecret: existing?.sharedSecret || null,
    impostor: false,
  };
  attachPendingToken(peerId);  // pulls sharedSecret from _pendingTokens if present
  saveFriendsData();
  if (state.activeFriendsView) renderFriendsSidebar();
}
...
export function handleIncomingFriendRequest(data, conn) {
  ...
  // Store the initiator's shared secret so it gets attached when we accept
  if (data.sharedSecret) {
    if (!state._pendingTokens) state._pendingTokens = {};
    state._pendingTokens[from] = data.sharedSecret;
  }
  if (!state.dms) state.dms = {};
...
function _dmRow(pid, name, isFri) {
  ...
  const impostor = isFri && state.friends?.[pid]?.impostor;
  const badge    = impostor
    ? `<div style="position:absolute;top:-3px;right:-3px;width:11px;height:11px;border-radius:50%;background:var(--red,#e06c75);color:#fff;font-size:7px;display:flex;align-items:center;justify-content:center;font-weight:700;border:1.5px solid var(--bg-deep)">!</div>`
    : '';
  return `<div class="channel-item${active ? ' active' : ''}" onclick="window._gmOpenDM('${pid}','${safeN}')">
    <div style="position:relative;width:22px;height:22px;flex-shrink:0">
      <div style="width:22px;height:22px;border-radius:50%;background:${color}${online?'20':'14'};border:1.5px solid ${impostor?'var(--red,#e06c75)':color+(active?'':'44')};color:${color};display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:700;transition:all .3s;${online?'':'filter:grayscale(0.8);opacity:0.55;'}">${(name||'?').charAt(0).toUpperCase()}</div>
      ${badge}
    ...
    ${impostor ? `<span title="Possible imposter" style="font-size:12px;flex-shrink:0">⚠️</span>` : ''}
...
export function renderFriendsGrid() {
  ...
  for (const { pid, name, friend } of all) {
    ...
    const unread   = state.dmUnread?.[pid] || 0;
    const online   = !!(state.peerConns[pid]?.conn?.open);
    const impostor = friend && state.friends?.[pid]?.impostor;
    ...
    card.style.cssText = `background:var(--bg-raised);border:1px solid ${impostor?'var(--red,#e06c75)':'var(--border)'};border-radius:var(--radius);padding:14px;cursor:pointer;position:relative;transition:border-color .15s;`;
    card.onmouseenter = () => card.style.borderColor = impostor ? 'var(--red,#e06c75)' : 'var(--border-accent)';
    card.onmouseleave = () => card.style.borderColor = impostor ? 'var(--red,#e06c75)' : 'var(--border)';
    ...
    card.innerHTML = `
      <div style="position:relative;width:44px;height:44px;margin-bottom:10px">
        <div style="width:44px;height:44px;border-radius:50%;background:${color}${online?'22':'18'};border:2px solid ${impostor?'var(--red,#e06c75)':color+(online?'55':'25')};color:${color};display:flex;align-items:center;justify-content:center;font-size:18px;font-weight:700;transition:all .3s;${avatarFilter}">${(name||'?').charAt(0).toUpperCase()}</div>
        ...
        ${impostor ? `<div style="position:absolute;top:-4px;right:-4px;width:16px;height:16px;border-radius:50%;background:var(--red,#e06c75);color:#fff;font-size:9px;display:flex;align-items:center;justify-content:center;font-weight:700;border:2px solid var(--bg-raised)">!</div>` : ''}
      </div>
      <div style="font-size:13px;font-weight:600;margin-bottom:3px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${escapeHtml(name||pid)}${impostor?' ⚠️':''}</div>
      <div style="font-size:11px;color:var(--text-muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${impostor ? '<span style="color:var(--red,#e06c75)">Possible imposter</span>' : escapeHtml(preview)}</div>
...
export function showPeerProfile(peerId, name) {
  ...
  const isImpostor = fri && state.friends[peerId]?.impostor;
  ...
  const impostorBanner = isImpostor
    ? `<div style="background:var(--red,#e06c75)22;border:1px solid var(--red,#e06c75);border-radius:6px;padding:8px 10px;font-size:11px;color:var(--red,#e06c75);display:flex;align-items:center;gap:6px;margin-bottom:12px">
         <span style="font-size:14px">⚠️</span>
         <span><strong>Possible imposter</strong> — this peer ID connected with an unexpected identity token</span>
       </div>`
    : '';

  const avatarBadge = isImpostor
    ? `<div style="position:absolute;top:-4px;right:-4px;width:18px;height:18px;border-radius:50%;background:var(--red,#e06c75);color:#fff;font-size:11px;display:flex;align-items:center;justify-content:center;font-weight:700;border:2px solid var(--bg-raised)">!</div>`
    : '';
  ...
  el.innerHTML = `<div class="modal" style="max-width:300px;padding:20px;position:relative" onclick="event.stopPropagation()">
    ...
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:18px">
      <div style="position:relative;flex-shrink:0">
        <div style="width:48px;height:48px;border-radius:50%;background:${color}22;border:2px solid ${isImpostor ? 'var(--red,#e06c75)' : color + '55'};color:${color};display:flex;align-items:center;justify-content:center;font-size:20px;font-weight:700">${(peerName||'?').charAt(0).toUpperCase()}</div>
        ${avatarBadge}
      </div>
      <div style="min-width:0">
